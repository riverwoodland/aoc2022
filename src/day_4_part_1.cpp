...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    while (std::getline(fileStream, line))
    {
        ...
        if (firstSection.first <= secondSection.first && firstSection.second >= secondSection.second)
            ++overlappingSections;
        else if (secondSection.first <= firstSection.first && secondSection.second >= firstSection.second)
            ++overlappingSections;
    }
